...
> 组长本地完善分支：`codex/review-risk-lab-mcp`。复核中补充了严格 JSON 权重校验、数据时间与逐项来源展示、组合样本和计算口径展示，并加强了界面与 E2E；尚未合并 `main`。
...
- 权重值必须是 JSON 数值；布尔值和数字字符串在 HTTP/MCP 边界直接拒绝，不能被隐式转换为权重。
...
- `/risk` 与 `/market` 均采用路由懒加载；最终风险 JS chunk 约 31.88 KB，gzip 约 11.19 KB。
- 风险页面包含 loading、empty、validation、error/retry、partial、fixture/fallback 和 success。
- 画像、资产和组合结果显示计算时间或数据截至时间；单资产表显示统计区间、状态与来源，组合结果显示共同样本数、成分来源和计算口径。
...
```text
D:\金融理财咨询智能Agent(行情查询、风险解读、理财建议)\.runtime\risk-review-1366x768.jpg
D:\金融理财咨询智能Agent(行情查询、风险解读、理财建议)\.runtime\risk-review-1920x1080.jpg
```
